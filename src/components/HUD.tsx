...
export const HUD = React.memo(() => {
  ...
  return (
    <div className="flex-none z-[100] bg-white pt-[calc(1rem+var(--safe-top-inset,env(safe-area-inset-top)))] pb-3 px-4 flex items-center justify-between w-full shadow-[0_2px_10px_rgba(0,0,0,0.03)] border-b border-zinc-100 relative">
      
      {/* 1. Track/Level (Flag Equivalent) */}
      <div className="flex items-center gap-1.5 cursor-pointer active:opacity-70 transition-opacity">
        <div 
          className="w-8 h-8 rounded-lg border-2 border-zinc-200 flex items-center justify-center bg-white shadow-sm"
        >
          <TrackIcon className="w-5 h-5" style={{ color: meta.color }} strokeWidth={2.5} />
        </div>
        <span className="font-black text-[15px] text-zinc-700">
          {level}
        </span>
      </div>
      ...
      >
        ...
        <span className="font-black text-[15px] text-[#1CB0F6]">
          {coins}
        </span>
      </div>
      ...
      >
        ...
        <span className="font-black text-[15px] text-[#FF4B4B]">
          {energy}
        </span>
      </div>
