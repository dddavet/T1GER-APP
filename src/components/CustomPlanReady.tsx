  ...
  return (
    <div className="w-full flex flex-col items-center justify-center space-y-6 mt-4">
      ...
      >
        ...
        <div className="bg-zinc-50 border border-zinc-200 rounded-full px-5 py-2 inline-block shadow-lg backdrop-blur-md">
          <span className="text-sm font-bold text-zinc-800">Alcanzarás el Rango Lobo en 45 días</span>
        </div>
      </motion.div>
      ...
      <motion.div 
        ...
        className="w-full bg-[#111113] border border-zinc-200 rounded-[2rem] p-6 shadow-2xl relative overflow-hidden"
      >
        <div className="absolute top-0 right-0 w-32 h-32 bg-[#FF6B00]/10 rounded-full blur-[40px]" />
        
        <div className="flex justify-between items-end mb-6">
          <div>
            <h3 className="text-lg font-bold text-zinc-800 leading-tight">Recomendación Diaria</h3>
            <span className="text-xs font-medium text-zinc-500">Ajustable en cualquier momento</span>
          </div>
        </div>

        <div className="grid grid-cols-2 gap-4">
          <DashboardCard icon={<Target size={16} className="text-zinc-800"/>} label="Misiones" value="3" subtitle="Por día" colorClass="text-zinc-800" dashArray="75, 100" />
...
const DashboardCard = ({ icon, label, value, subtitle, colorClass, dashArray }: { icon: React.ReactNode, label: string, value: string, subtitle: string, colorClass: string, dashArray: string }) => (
  <div className="bg-zinc-50 border border-zinc-200 rounded-2xl p-4 flex flex-col items-center justify-center relative shadow-inner">
    <div className="absolute top-3 left-3">
      {icon}
    </div>
    <span className="text-xs font-bold text-zinc-500 mb-1 ml-4">{label}</span>
