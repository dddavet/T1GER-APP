...
export const PromptInputBox = React.forwardRef<HTMLDivElement, PromptInputBoxProps>((props, ref) => {
  const { onSend = () => {}, isLoading = false, placeholder = "Escribe tu duda o desafío táctico...", className } = props;
  ...
  const handleSubmit = () => {
    if (input.trim() || files.length > 0) {
      let messagePrefix = "";
      if (showSearch) messagePrefix = "[Búsqueda: ";
      else if (showThink) messagePrefix = "[Razonamiento: ";
      else if (showCanvas) messagePrefix = "[Lienzo: ";
  ...
  return (
    <>
      <PromptInput
        ...
        className={cn(
          "w-full bg-[#141418] border-[#2A2A32] shadow-[0_8px_30px_rgba(0,0,0,0.4)] transition-all duration-300 ease-in-out ring-1 ring-white/5",
          ...
          <PromptInputTextarea
            placeholder={
              showSearch
                ...
                : showThink
                ? "Razonar estrategia profunda..."
                : showCanvas
                ? "Crear en lienzo de negocios..."
                : placeholder
            }
          ...
          >
            ...
            <div className="flex items-center">
              <button
                ...
                className={cn(
                  "rounded-full transition-all flex items-center gap-1 px-2.5 py-1 border h-7 cursor-pointer",
                  showSearch
                ...
                <AnimatePresence>
                  {showSearch && (
                    <motion.span
                      ...
                      className="text-xs overflow-hidden whitespace-nowrap text-[#1EAEDB] flex-shrink-0 font-bold"
                    >
                      Búsqueda
                    </motion.span>
                  )}
                ...
                <AnimatePresence>
                  {showThink && (
                    <motion.span
                      ...
                      exit={{ width: 0, opacity: 0 }}
                      transition={{ duration: 0.2 }}
                      className="text-xs overflow-hidden whitespace-nowrap text-[#8B5CF6] flex-shrink-0 font-bold"
                    >
                      Razonar
                    </motion.span>
                  )}
              ...
              <button
                ...
                className={cn(
                  "rounded-full transition-all flex items-center gap-1 px-2.5 py-1 border h-7 cursor-pointer",
                  showCanvas
                    ? "bg-[var(--ob-accent)]/20 border-[var(--ob-accent)] text-[var(--ob-accent)]"
                    : "bg-transparent border-transparent text-[#9CA3AF] hover:text-white"
                )}
              >
                <div className="w-4 h-4 flex items-center justify-center flex-shrink-0">
                  <motion.div
                    ...
                    transition={{ type: "spring", stiffness: 260, damping: 25 }}
                  >
                    <FolderCode className={cn("w-3.5 h-3.5", showCanvas ? "text-[var(--ob-accent)]" : "text-inherit")} />
                  </motion.div>
                </div>
                <AnimatePresence>
                  {showCanvas && (
                    <motion.span
                      ...
                      exit={{ width: 0, opacity: 0 }}
                      transition={{ duration: 0.2 }}
                      className="text-xs overflow-hidden whitespace-nowrap text-[var(--ob-accent)] flex-shrink-0 font-bold"
                    >
                      Lienzo
                    </motion.span>
                  )}
          ...
          >
            <Button
              ...
              className={cn(
                ...
                isRecording
                  ? "bg-transparent hover:bg-red-500/20 text-red-500 hover:text-red-400"
                  : hasContent
                  ? "bg-[var(--ob-accent)] hover:bg-[var(--ob-accent)]/80 text-black font-black"
                  : "bg-white/10 hover:bg-white/20 text-[#9CA3AF] hover:text-white"
              )}
              onClick={() => {
            ...
            >
              {isLoading ? (
                ...
                <ArrowUp className="h-4 w-4 text-black stroke-[3]" />
              ) : (
                <Mic className="h-4 w-4 text-zinc-300 transition-colors" />
              )}
            </Button>
          </PromptInputAction>
        </PromptInputActions>
