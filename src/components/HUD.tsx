  ...
  return (
    <header className="z-40 flex w-full flex-none items-center justify-between border-b border-white/6 bg-[#071C19]/92 px-5 pb-3 pt-[calc(.8rem+env(safe-area-inset-top))] backdrop-blur-xl">
      <div className="flex items-center gap-2.5">
        ...
        <div><span className="block text-sm font-semibold tracking-[-.02em] text-white">T1GER</span><span className="block text-[10px] text-[#64877F]">Investing · Level {appUser?.level || 1}</span></div>
      </div>
      <div className="flex items-center gap-2">
        ...
        <div className="flex h-9 items-center gap-1.5 rounded-xl bg-[#3FC78E]/9 px-2.5 font-mono text-xs text-[#78DDB0]" aria-label={`${stats.verifiedXP} verified XP`}><ShieldCheck size={16} />{stats.verifiedXP}</div>
      </div>
    </header>
  );
});
